...
# Load Data
try:
    # Fix: Explicitly load headers and force numeric conversion
    df = pd.read_csv("benchmark_results.csv")
    
    # Force "Speed_MBps" to be numeric (turns text headers into NaN)
    df["Speed_MBps"] = pd.to_numeric(df["Speed_MBps"], errors='coerce')
    
    # Drop bad rows
    df = df.dropna(subset=["Speed_MBps"])

except FileNotFoundError:
    st.error("❌ No data found! Run 'benchmark.py' first.")
    st.stop()
...
# Calculate averages
avg_speeds = df.groupby(["Provider", "Operation"])["Speed_MBps"].mean().unstack()
...
if not avg_speeds.empty:
    try:
        ...
        with col3:
            # Lock-In Risk: Who has the slowest download?
...
with col_chart1:
    ...
    if not df[df["Operation"] == "Upload"].empty:
        fig_up = px.bar(
            df[df["Operation"] == "Upload"], 
            x="Provider", 
            y="Speed_MBps", 
            color="Provider",
            title="Upload Speed (MB/s) - Higher is Better",
            text_auto='.2f'
        )
...
with col_chart2:
    ...
    if not df[df["Operation"] == "Download"].empty:
        fig_down = px.bar(
            df[df["Operation"] == "Download"], 
            x="Provider", 
            y="Speed_MBps", 
            color="Provider",
            title="Download Speed (MB/s) - Higher is Better",
            text_auto='.2f'
        )
...
pricing = {
    ...
    "Google": {"storage": 0.023, "egress": 0.12} 
}
...
if cost_data:
    ...
    # FIX: Apply formatting ONLY to the number columns
